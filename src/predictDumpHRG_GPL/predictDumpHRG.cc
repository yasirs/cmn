...
struct ioparameters {
	...
	string		dumpfolder;		// folder to dump dendograms and predictions
...
bool parseCommandLine(int argc, char * argv[]) {
	...
	if (argc==1) {
		cout << "\n  -- Hierarchical Random Graphs : Predict Missing Edges and dump files--\n";
		...
		cout << "  -d <directory>  (optional) Folder to dump the sampled and best dendograms, and ther predictions (default: dump/)\n";
		cout << "\n";
		...
		while (argct < argc) {
			...
			} else if (temp == "-d") { argct++; ioparm.dumpfolder = argv[argct]; 
			} else if (temp == "-f") {
				argct++;
				temp = argv[argct];
				ext = ".pairs";
...
void recordSampledHRG(const int sample, const double thisL) {
	...
	fout << "StartTime      : " << ioparm.start_time;
	fout << "InputFile      : " << ioparm.f_pairs   << "\n";
	fout << "Dump Directory : " << ioparm.dumpfolder << "\n";
	fout << "---Basic-Graph-Information---\n";
	fout << "Nodes          : " << ioparm.n		<< "\n";
	fout << "Edges          : " << ioparm.m/2	<< "\n";
	fout << "---HRG-Information---\n";
